...
def setDeviceId():
  ...
  id = raw_input("deviceId ["+deviceId+"]: ")
  if id:
    deviceId = id
...
def set_speed():
  ...
  if client.security.access_token is None:
    ...
    loop = True
    while loop:
      speedStr = raw_input("Speed [0..100] (silent:40, medium:60, high:80):")
      try:
        speed = int(speedStr)
      except ValueError:
...
def set_humidity():
  ...
  if client.security.access_token is None:
    ...
    loop = True
    while loop:
      humStr = raw_input("Target Humidity [30..70]:")
      try:
        hum = int(humStr)
      except ValueError:
...
def set_mode():
  ...
  if client.security.access_token is None:
    ...
    loop = True
    while loop:
      modeStr = raw_input("Mode [1..4] (1: TARGET_MODE, 2: CONTINUOUS_MODE, 3: SMART_MODE, 4: DRYER_MODE): ")
      try:
        mode= int(modeStr)
      except ValueError:
        print "input value is not a valid integer, retry."
...
def main(argv):
  ...
  while True:
    menu()
    cmd = raw_input("Choose a command: ")
    if cmd == "0":
      #Perform login
      do_login()
    elif cmd == "1":
